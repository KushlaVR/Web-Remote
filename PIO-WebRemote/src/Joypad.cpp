...
bool Joypad::processParcel(JsonString *json)
{
	...
	if (fieldsIndex >= 0)
	{
		...
		int arrayEnd = str.indexOf("]");
		if (fieldsIndex >= 0)
		{
			str = str.substring(fieldsIndex + 1, arrayEnd);
			fieldsIndex = str.indexOf("\"");
			while (fieldsIndex >= 0)
			{
				fieldsIndex++;
			...
			fieldsIndex = json->indexOf("readonlyFields");
			if (fieldsIndex >= 0)
			{
				str = json->substring(fieldsIndex + 16);
				fieldsIndex = str.indexOf("[");
				arrayEnd = str.indexOf("]");
				if (fieldsIndex >= 0)
				{
					Collection *readonlyFields = new Collection();
					str = str.substring(fieldsIndex + 1, arrayEnd);
					fieldsIndex = str.indexOf("\"");
					...
					while (fieldsIndex >= 0)
					{
						fieldsIndex++;
						int endIndex = str.indexOf("\"", fieldsIndex);
						Joypadfield *f = getFieldByName(str.substring(fieldsIndex, endIndex));
						f->isReadonly = true;
						fieldsIndex = endIndex + 1;
						fieldsIndex = str.indexOf("\"", endIndex + 1);
						Serial.println(f->name);
...
Joypadfield *Joypad::getFieldByName(String name)
{
	if (fields == nullptr)
		return 0;
...
void JoypadCollection::setValue(String name, double value)
{
	...
	// Set value for global storage of name/values pair
	if (!JoypadCollection::setValue(fields, name, value))
	{
		// If field not fount -> create newone
		Joypadfield *jf = new Joypadfield(name);
		jf->value = value;
		fields->add(jf);
	};

	// populate values for all connected clients
	Joypad *j = (Joypad *)getFirst();
	while (j != nullptr)
	{
...
void JoypadCollection::loop()
{
	...
	while (j != nullptr)
	{
		...
		{ // nothing changed, just report alive
			if (!j->keepAlive())
			{
				// report alive
				Joypad *next = (Joypad *)(j->next);
